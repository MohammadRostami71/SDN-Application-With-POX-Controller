"Mohammad Rostami - Pouya Sabeti"
"Cloud Computing Course in SBU"
...
class ClosTopo(Topo):
	...
	for switch in range(cores):
	    core.append(self.addSwitch('c%s'%(numberState)))
	    numberState = numberState + 1
	
	
	#Build Aggregation Switch  
	for c in range(cores):
	    for switch in range(fanout):
	    	aggre.append(self.addSwitch('a%s'%(numberState)))
	    	numberState = numberState + 1
	...
	for a in aggre:
            for switch in range(fanout):
                edge.append(self.addSwitch('e%s'%(numberState)))
                numberState = numberState + 1
	
	    
	        
	#Add Link Between Core Switch & Aggregation Switch
        ...
        for e in edge:
            for host in range(2):
                t=self.addHost('h%s'%(numberState))
                self.addLink(e, t)
                numberState = numberState + 1
